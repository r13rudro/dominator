...
int condition(char *line, int *i)
{
    ...
    char op;
    skip(line, &b);
    if (isdigit((unsigned char)line[b]))
        l = num_in(line, &b);
    else if (isalpha((unsigned char)line[b]))
